...
def has_matching_statement(
    principal: Node,
    effect_value: str,
    action_to_check: str,
    resource_to_check: str,
    condition_keys_to_check: CaseInsensitiveDict,
) -> bool:
    ...
    for policy in principal.attached_policies:
        if policy_has_matching_statement(
            policy,
            effect_value,
            action_to_check,
            resource_to_check,
...
def policy_has_matching_statement(
    ...
    for statement in _listify_dictionary(base_obj["Statement"]):
        ...
        if "Resource" in statement:
            for resource in _listify_string(statement["Resource"]):
                if _matches_after_expansion(
                    resource_to_check, resource, condition_keys_to_check
                ):
            ...
            for notresource in _listify_string(statement["NotResource"]):
                if _matches_after_expansion(
                    resource_to_check, notresource, condition_keys_to_check
                ):
        ...
        if "Condition" in statement:
            matches_condition = _get_condition_match(
                statement["Condition"], condition_keys_to_check
            )
        else:
...
def _get_condition_match(
    condition: Dict[str, Dict[str, Union[str, List]]], context: CaseInsensitiveDict
) -> bool:
    """
            ...
            if block.startswith("ForAllValues:"):
                ...
                for policy_context_key in condition[block]:
                    if not _get_str_match(
                        block,
                        policy_context_key,
                        condition[block][policy_context_key],
                        context,
                    ):
                        return False
            ...
            if block.startswith("ForAllValues:"):
                ...
                for policy_key in condition[block]:
                    if policy_key in context.keys():
                        for context_value in _listify_string(context[policy_key]):
                            if context_value != "":
                                if not _get_straight_str_match(
                                    block,
                                    policy_key,
                                    condition[block][policy_key],
                                    context,
            ...
            if block.startswith("ForAllValues:"):
                ...
                for policy_key in condition[block]:
                    if policy_key in context.keys():
                        for context_value in _listify_string(context[policy_key]):
                            if context_value != "":
                                if not _get_null_match(
                                    policy_key, condition[block][policy_key], context
                                ):
                                    return False
            elif block.startswith("ForAnyValue:"):
                # fail to match unless at least one of the provided context values match
...
def _get_str_match(
    block: str,
    policy_key: str,
    policy_value: Union[str, List[str]],
    context: CaseInsensitiveDict,
) -> bool:
...
def _convert_timestamp_to_datetime_obj(timestamp: str):
    ...
    if (
        "-" in timestamp
    ):  # policy simulator behavior: datetimestamps need dashes, even though ISO 8601 doesn't (?)
        ...
        return dt.datetime.fromtimestamp(
            float(timestamp), dt.timezone.utc
        )  # TODO: concern around float imprecision
...
def _get_null_match(
    policy_key: str, policy_value: Union[str, List[str]], context: CaseInsensitiveDict
) -> bool:
...
def resource_policy_matching_statements(
    node_or_service: Union[Node, str],
    ...
    for statement in _listify_dictionary(resource_policy["Statement"]):
        matches_principal, matches_action, matches_resource, matches_condition = (
            False,
            False,
            False,
            False,
        ...
        if "Principal" in statement:  # should be a dictionary
            ...
            if isinstance(node_or_service, Node):
                ...
                if "Service" in statement["Principal"]:
                    if node_or_service in _listify_string(
            ...
            if isinstance(node_or_service, Node):
                if "AWS" in statement["NotPrincipal"]:
                    if _principal_matches_in_statement(
                        node_or_service,
                        _listify_string(statement["NotPrincipal"]["AWS"]),
                    ):
                        matches_principal = False
            else:
                if "Service" in statement["NotPrincipal"]:
                    if node_or_service in _listify_string(
                        statement["NotPrincipal"]["Service"]
                    ):
                        matches_principal = False
        ...
        matches_action = _statement_matches_action(
            statement, action_to_check, condition_keys_to_check, True
        )
        if not matches_action:
            continue

        # if action is good, proceed to check resource
        matches_resource = _statement_matches_resource(
            statement, resource_to_check, condition_keys_to_check
        )
        if not matches_resource:
            continue
        ...
        if arns.get_account_id(node_or_service.arn) != resource_owner:
            ...
            for statement in matching_statements:
                if "NotPrincipal" in statement:
                    ...
                    isinstance(statement["Principal"], str)
...
def _principal_matches_in_statement(principal: Node, aws_principal_field: list):
    ...
    for value in aws_principal_field:
        if principal.arn == value:
            ...
            principal_root_str = "arn:{}:iam::{}:root".format(
                arns.get_partition(principal.arn), arns.get_account_id(principal.arn)
            )
...
def policies_include_matching_allow_action(
    principal: Node, action_to_check: str
) -> bool:
    ...
    if "Action" in statement:
        for item in _listify_string(statement["Action"]):
            if not is_resource_policy_check:
                ...
                if (
                    action.startswith("SNS")
                    or action.startswith("SQS")
                    or item.startswith("SNS")
                    or item.startswith("SQS")
                ):
                    action_parts = action.split(":")
                    item_parts = action.split(":")
                    new_action = "{}:{}".format(
                        action_parts[0].lower(), action_parts[1]
                    )
...
def _statement_matches_resource(
    statement: dict, resource: str, condition_keys: Optional[CaseInsensitiveDict] = None
) -> bool:
    ...
    if condition_keys is not None:
        for k, v in condition_keys.items():
            if isinstance(v, list):
                v = str(
                    v
                )  # TODO: how would a multi-valued context value be handled in resource fields?
            full_key = "${" + k + "}"
