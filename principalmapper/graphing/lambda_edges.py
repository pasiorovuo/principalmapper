...
from principalmapper.querying.local_policy_simulation import (
    resource_policy_authorization,
    ResourcePolicyEvalResult,
)
from principalmapper.querying import query_interface
from principalmapper.util import arns, botocore_tools, split_list
...
class LambdaEdgeChecker(EdgeChecker):
    ...
    def __init__(self, session: Session):
        ...
        self.function_list: List[Dict[str, Any]] = []
    ...
    def fetch_data(
        ...
        """Fetches data required for edge identification and stores it in the class instance."""
    ...
    def return_edges(
        ...
        region_allow_list: Optional[List[str]] = None,
        region_deny_list: Optional[List[str]] = None,
        scps: Optional[List[List[Policy]]] = None,
        client_args_map: Optional[dict] = None,
    ) -> List[Edge]:
        """Fulfills expected method return_edges. If session object is None, runs checks in offline mode."""

        self.fetch_data(region_allow_list, region_deny_list, client_args_map)

        logger.info("Generating Edges based on Lambda data.")
        logger.debug(
            "Identified {} Lambda functions for processing".format(
                len(self.function_list)
            )
        )

        return self._parallelize(
            [n for n in nodes if ":role/" in n.arn],
            logger,
            generate_edges_locally,
            self.function_list,
            scps,
        )
    ...
    for node_destination in nodes:
        ...
        sim_result = resource_policy_authorization(
            ...
            (
                ...
                if node_destination and node_destination.trust_policy is not None
                else {}
            ),
            "sts:AssumeRole",
        ...
        for node_source in nodes:
            ...
            can_pass_role, need_mfa_passrole = (
                query_interface.local_check_authorization_handling_mfa(
                    node_source,
                    "iam:PassRole",
                    node_destination.arn,
            ...
            if can_pass_role:
                can_create_function, need_mfa_0 = (
                    query_interface.local_check_authorization_handling_mfa(
                        node_source,
                ...
                if can_create_function:
                    if need_mfa_0 or need_mfa_passrole:
                        reason = (
                            "(requires MFA) can use Lambda to create a new function with arbitrary code, "
                            "then pass and access"
                        )
                    else:
                        reason = "can use Lambda to create a new function with arbitrary code, then pass and access"
                    new_edge = Edge(node_source, node_destination, reason, "Lambda")
